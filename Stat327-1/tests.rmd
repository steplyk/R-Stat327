...
prop.test(91, 220, .14)
```
***Fail to reject the null because the p-value is not statiscally significant.***
...
m = matrix(data=c(1, 2, 1, 2), nrow=2, ncol=2, byrow=TRUE)
layout(m)

boxplot(before, main = "Before")
boxplot(after, main ="After")
boxplot(diff, main = "Difference")
```
...
t.test(diff, mu=0)
```
***Fail to reject the null hypothesis because the p-value is very small, therefore statisically significant.***
...
mosaicplot(table(wine), main = "Type of Wine vs Type of Music")
```
...
t.test(table(wine))
```
***There's strong evidence the music played and wine purchased are not independentof eachother, therefore we fail to reject the null.***
...
```{r}
boxplot(CO2$uptake ~ CO2$Type, ylim = c(0,50))
```
...
```
***Reject the null hypothesis that the mean uptake for the grass in Mississippi and Quebec is the same.***
...
```{r}
t.test(CO2$uptake ~ CO2$Type, conf.level = .99)
```
