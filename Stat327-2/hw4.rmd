echo "# R-Stat327" >> README.md
git init
git add README.md
git commit -m "first commit"
git remote add origin https://github.com/steplyk/R-Stat327.git
...
land = data.frame(tables[2])
land = land[-c(51),]
land = with(land, land[order(land$NULL.State),])
land = setNames(land, c("rank","state","km","area"))
land.a = land[,4]
land.area = factor(land.a)
land.area = gsub("," , "", land.area)
land.area = as.numeric(land.area)
```
...
farm = setNames(farm, c("state", "farm"))
f = farm[,2]
```
...
```{r}
new <- merge(farm, land, all.x=T)
n = subset(new, select = -c(3:4))
```

Make a scatterplot of y = farm area vs. x = land area.
```{r}
plot(x = land.a, y = f, xlab = "land area", ylab = "farm area")
```
...
```{r}
plot(land.a, f)
identify(x=land.a, y=f, plot=TRUE)
```
...
pg = grep(pattern = "/title/tt0111161", x = imdb.page) 
imdb.lines = imdb.page[pg]
par = htmlParse('http://www.imdb.com/chart/top');
movies.urls = xpathSApply(par, "//td[@class='titleColumn']//a", xmlAttrs)[1, ]
producer.list=list()

for (movie.url in movies.urls[1:250]) {
  print(movie.url)
  title=sub('.*/title/(.*)/.*', '\\1', movie.url)
  imdb.url = paste(sep="", "http://www.imdb.com/title/", title,"/fullcredits")
  print(imdb.url)
  movie.tables = readHTMLTable(imdb.url)
  movie.producers = as.character(movie.tables[[4]]$V1)
  movie.producers = producer.list[length(producer.list)+1]
  name.list = unlist(producer.list)
}

tabled.name=table(name.list)
head(sort(tabled.name,decreasing=TRUE),5)
tail(sort(tabled.name),5)
```
